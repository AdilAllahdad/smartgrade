...
const StatCard = ({ title, value, icon: Icon, color, bgColor, iconColor, onClick, isActive = false }) => (
    <div 
        className={`bg-gradient-to-br ${bgColor} rounded-lg shadow-md p-6 border-l-4 ${color} 
                  hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1 
                  ${onClick ? 'cursor-pointer' : ''}
                  ${isActive ? 'ring-4 ring-offset-2 ring-blue-300' : ''}`}
        onClick={onClick}
    >
        <div className="flex items-center">
            <div className={`flex-shrink-0 p-3 rounded-full ${iconColor} bg-opacity-20`}>
