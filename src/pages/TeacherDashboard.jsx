...
export default function TeacherDashboard() {
    ...
    const StatCard = ({ title, value, icon: Icon, color, bgGradient }) => (
        <motion.div
            ...
            className={`bg-white rounded-lg sm:rounded-xl shadow-lg p-4 sm:p-6 ${bgGradient} overflow-hidden relative`}
        >
            <div className="absolute top-0 right-0 w-16 h-16 sm:w-24 sm:h-24 opacity-10 transform translate-x-6 sm:translate-x-8 -translate-y-6 sm:-translate-y-8">
            ...
            <div className="flex items-center relative z-10">
                ...
                <div className="ml-3 sm:ml-5">
                    ...
                    <div className="mt-1 text-xl sm:text-2xl md:text-3xl font-bold text-gray-900">{value}</div>
                </div>
            </div>
        </motion.div>
    );
    ...
    const UploadModal = () => (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center p-2 sm:p-4 z-50 backdrop-blur-sm">
            <motion.div
                ...
                className="bg-white rounded-lg sm:rounded-xl p-4 sm:p-8 max-w-lg w-full shadow-2xl border border-gray-100"
            >
                ...
                <div className="mt-4 sm:mt-6 flex flex-col sm:flex-row justify-end gap-2 sm:gap-4">
                    <button
                        onClick={() => setShowUploadModal(false)}
                        className="px-4 sm:px-5 py-2 sm:py-2.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 shadow-sm transition-all w-full sm:w-auto"
                    >
                        Cancel
                    </button>
                    <button
                        onClick={() => setShowUploadModal(false)}
                        className={`px-4 sm:px-5 py-2 sm:py-2.5 text-sm font-medium text-white rounded-lg shadow-sm transition-all w-full sm:w-auto ${uploadType === 'answerSheet' ? 'bg-blue-600 hover:bg-blue-700' : 'bg-purple-600 hover:bg-purple-700'}`}
                    >
                        Upload
                    </button>
                </div>
    ...
    return (
        <DashboardLayout allowedRoles={['teacher']}>
            ...
            <motion.div 
                ...
                initial="hidden"
                animate="show"
                className="space-y-6 sm:space-y-8 md:space-y-12 px-2 sm:px-0">
                {/* Welcome Banner */}
                <motion.div 
                    initial={{ opacity: 0, y: -20 }}
                    animate={{ opacity: 1, y: 0 }}
                    className="bg-gradient-to-r from-blue-500 to-indigo-600 rounded-xl sm:rounded-2xl p-4 sm:p-6 md:p-8 text-white shadow-lg"
                >
                    <h1 className="text-xl sm:text-2xl md:text-3xl font-bold mb-2">Welcome back, Teacher</h1>
                    <p className="text-sm sm:text-base opacity-90">Manage your exams, answer sheets and student evaluations all in one place</p>
                </motion.div>
                ...
                <div>
                    <div className="flex items-center justify-between mb-4 sm:mb-6">
                        ...
                        <span className="text-xs sm:text-sm text-blue-600 font-medium hover:underline cursor-pointer">View Details</span>
                    </div>
                ...
                <div>
                    <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-gray-900 mb-3 sm:mb-4">Format Templates</h2>
                    <p className="text-sm sm:text-base text-gray-600 mb-4 sm:mb-6">Please use these standardized templates when creating exam papers and answer sheets for better consistency and evaluation.</p>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                        <motion.button 
                            onClick={() => downloadFile(TEMPLATES.QUESTION_PAPER.url, TEMPLATES.QUESTION_PAPER.fileName)}
                            whileHover={{ scale: 1.02 }}
                            whileTap={{ scale: 0.98 }}
                            className="group flex items-center p-4 sm:p-6 bg-gradient-to-br from-indigo-50 to-white rounded-lg sm:rounded-xl shadow-md hover:shadow-lg transition-all border border-indigo-100"
                        >
                            <div className="p-3 sm:p-4 rounded-full bg-indigo-600 text-white group-hover:bg-indigo-700 transition-colors flex-shrink-0">
                                <DocumentArrowDownIcon className="h-5 w-5 sm:h-6 sm:w-6 md:h-7 md:w-7" />
                            </div>
                            <div className="ml-3 sm:ml-6">
                                <h3 className="text-sm sm:text-base md:text-lg font-bold text-gray-900 mb-1">Question Paper Format</h3>
                                <p className="text-xs sm:text-sm text-gray-600">Download the standard question paper format template</p>
                            </div>
                        </motion.button>
                        ...
                        <motion.button 
                            ...
                            whileHover={{ scale: 1.02 }}
                            whileTap={{ scale: 0.98 }}
                            className="group flex items-center p-4 sm:p-6 bg-gradient-to-br from-cyan-50 to-white rounded-lg sm:rounded-xl shadow-md hover:shadow-lg transition-all border border-cyan-100"
                        >
                            <div className="p-3 sm:p-4 rounded-full bg-cyan-600 text-white group-hover:bg-cyan-700 transition-colors flex-shrink-0">
                                <DocumentCheckIcon className="h-5 w-5 sm:h-6 sm:w-6 md:h-7 md:w-7" />
                            </div>
                            <div className="ml-3 sm:ml-6">
                                <h3 className="text-sm sm:text-base md:text-lg font-bold text-gray-900 mb-1">Answer Sheet Format</h3>
                                <p className="text-xs sm:text-sm text-gray-600">Download the special answer sheet format template</p>
                            </div>
                        </motion.button>
                    </div>
                </div>
                ...
                >
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-0 mb-4 sm:mb-6">
                        <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-gray-900">Upload Exam Papers</h2>
                        <div className="bg-blue-50 text-blue-700 px-3 sm:px-4 py-1.5 sm:py-2 rounded-full text-xs sm:text-sm font-medium w-fit">
                            New Feature
                        </div>
                    </div>
                ...
                >
                    <div className="bg-white rounded-lg sm:rounded-xl shadow-md p-4 sm:p-6 border border-gray-100">
                        <div className="mb-4 bg-blue-50 border-l-4 border-blue-500 p-3 sm:p-4 rounded">
